...
export default function Conversation() {
    ...
    const [initialMessage, setInitialMessage] = useState<DataItem | undefined>(undefined)
    ;

    const [answers, setAnswers] = useState<string[] | undefined>(undefined);

    useEffect(() => {
        const startMessage = messagesData.find(message => message.name === "Start");
    ...
    return (
        ...
        <>
            <div>
                <img className="rounded-circle" src="https://placehold.co/50x50" alt="Photo de inconnu.e"/>
                <p>Inconnu.e</p>
            </div>

            {initialMessage && (
                <Message text={extractMessageAndAnswers(initialMessage.source)?.message || ''}/>
            )}
